...
# Battle 首页标题与房间纯文字信息验收

## 验收对象

- 版本：`bbc769c`
- 环境：Vercel Production `dpl_AHcCUvQd2zH5qtkLmDyigi7oaweo`，稳定别名 `final-tma-pi.vercel.app`，状态 `READY`
- 页面状态：Telegram Mini App、游戏首页、默认选中 100 K-coin
- 用户结果：主视觉标题与副标题居中；删除“固定档位”“选择本场入场费”和规则说明；三档房间卡片顶部居中且只显示“奖池金额：{双方奖池}　门票：{每人入场费} K-coin”纯文字

## 视觉依据与实现证据

- 用户提供的标题区域来源：`/var/folders/__/ffcc9r1113l4c8cd1z4m8tp80000gn/T/codex-clipboard-30b8480c-fb47-45d5-a2c9-e05f6bd59255.png`，`367 × 172px`
- 用户提供的旧房间信息来源：`/var/folders/__/ffcc9r1113l4c8cd1z4m8tp80000gn/T/codex-clipboard-85d2801b-4407-4644-9614-13e916e75d15.png`，`228 × 133px`
- 最终顶部真机截图：`/Users/mac/.codex/visualizations/2026/08/02/019fc0a5-857e-7392-941d-1255d0f49371/battle-home-copy-final-top.png`，`420 × 715px`
- 最终底部真机截图：`/Users/mac/.codex/visualizations/2026/08/02/019fc0a5-857e-7392-941d-1255d0f49371/battle-home-copy-final-bottom.png`，`420 × 715px`
- 聚焦同屏对照：`/Users/mac/.codex/visualizations/2026/08/02/019fc0a5-857e-7392-941d-1255d0f49371/battle-home-copy-comparison.png`，`860 × 715px`；左侧为用户旧房间信息来源，右侧为最终 Telegram 页面

## 视口与归一化

- 实现截图来自 macOS Telegram Mini App 窗口，物理截图为 `420 × 715px`；该宿主未暴露网页 CSS 视口和 `deviceScaleFactor`，因此不虚构 CSS 尺寸或像素密度。
- 聚焦同屏对照把 `228 × 133px` 来源等比缩放到 `420px` 宽，最终实现保持 `420 × 715px` 原始尺寸，两者在同一画布中并排检查信息层变化。
- 来源截图用于标识必须删除的旧房间文案与数值背景；最终视觉真相同时包含用户在本任务中明确确认的纯文字内容和金额映射。

## 必检保真面

- 字体与文案：`1v1 BATTLE` 与“胜者拿走奖池”整体居中；20、100、500 三档分别显示“奖池金额：40　门票：20 K-coin”“奖池金额：200　门票：100 K-coin”“奖池金额：1000　门票：500 K-coin”，没有截断或换行。
- 间距与布局：主视觉内容保持原卡片高度和像素对战构图；三档信息固定在房间卡片顶部中央，选中勾位于右下角，不遮挡文字；500 档和统一选择按钮可完整滚动到达。
- 色彩与视觉令牌：沿用原房间位图、橙色选中描边和白色高对比文字；新文字没有背景色、玻璃层、边框或色块。
- 图片质量：三张既有独立 WebP 房间图保持原比例、清晰度和裁切，没有新增、替换或拉伸素材。
- 内容与交互：旧“固定档位”“选择本场入场费”“胜者获得双方奖池的 90%；平局原额退回。”“每人入场”“双方奖池”“胜者到账”均不存在；点击 20 档后 `aria-pressed` 与底部按钮立即切换到 20 K-coin，随后恢复默认 100 档，原 `onChooseTier` 流程未改变。

## Findings

- 没有发现可执行的 P0、P1 或 P2 视觉问题。
- Telegram 宿主控制面不提供浏览器控制台，因此未取得控制台日志；页面没有显示运行时错误、失败覆盖层或资源缺失。

## 比较历史

1. 首次部署后 Telegram WebView 仍显示旧缓存；重新加载 Mini App 后载入 `bbc769c` 对应新界面。
2. 新界面首次同屏比较即满足标题居中、旧区域删除、三档纯文字、无背景和可滚动可达要求，未产生需要再次修正的 P0、P1 或 P2 问题。
...
- [x] 删除档位区旧标题和规则说明。
- [x] 三档卡片顶部居中显示唯一纯文字信息。
- [x] 保持三档选择、统一按钮、金额数据来源和后端链路不变。
- [x] `pnpm --filter @pokepets/web build` 与 `git diff --check` 通过。
- [x] Vercel Production 自动部署 READY，Telegram Mini App 真机视觉与选择状态通过。
