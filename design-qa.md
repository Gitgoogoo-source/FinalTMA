...
参考图的“左侧信息与操作、右侧独立主形象、底部四列选择区”层级已保留。主形象继续限制在右侧网格列内，资源、尺寸、位置和裁切均不调整；依据用户最新裁决，左侧名称、属性和四个操作入口删除自身填充背景、圆角边框与阴影，只保留文字、图标、触控区域和既定排列。底部选择区通过独立分隔线与详情区分开，两个区域没有边界重叠。
