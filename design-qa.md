...
- 五页共用的头像、TON、K-coin、Gems 和刷新入口整体位于原生控件行下方，没有遮挡或重叠。
...
- 用户目标：保留现有三宠主图和布局，将皇冠、每日免费稀有盲盒蛋、每日 `100 Gems` 水晶从简易线框图标改为与选定设计一致的真实暖金日漫游戏素材
...
- 稀有盲盒蛋具有完整金色外框、琥珀核心和装饰底座；`100 Gems` 使用独立切面水晶、金色光环与装饰底座，二者不再呈现为单线图标。
...
- 文案与业务内容：继续显示真实 `199 Stars · 30 天`、每日 `100 Gems`、每日免费稀有盲盒、交易手续费返还及动态购买/续费状态；主图不固化任何价格和状态文案。
...
- 文案与业务内容：保留实时 `199 Stars · 30 天`、每日 `100 Gems`、每日免费稀有盲盒、交易手续费返还和购买按钮；图片本身不固化价格或状态。
...
- 用户结果：保留暖橙金三宠主图；固定英文标题 `VIP MONTHLY PASS`；突出每日免费稀有盲盒和每日 `100 Gems`；将手续费返还作为辅助权益；购买入口采用开盒“开 10 次”同款暖橙双行大圆角按钮
...
- 两项权益由白色大卡改为金色蛋／宝石主体加独立小标签，文案分别固定为“每日免费稀有盲盒”和“每日 100 Gems”。
...
- 两枚图标统一为奶油白、香槟金和琥珀橙的紧凑 3D 日漫游戏素材；稀有盲盒使用象牙白蛋壳、琥珀核心和小型冠饰，`Gems` 使用单枚对称切面水晶。
- 删除旧图标的大光环、花枝、丝带和外扩底座。主体只占正方形画布上方区域，底部固定保留透明安全区，供现有 `-8px` 标签叠层进入而不遮挡图标主体。
- 两个项目成品均为 `384 × 384px` 透明 WebP；稀有盲盒蛋 SHA-256 为 `82ed7df02b06d5ed6318b27d942e96b2d371ffd886e5b40f11e1eb6caef15adf`，Gems 水晶 SHA-256 为 `6915c4e9e708a10cacc942a5ca1e54f07a8ae2bd5f57ee5ddc6b079cf63f3fd1`。
...
- 版本：`37d091c`
- 页面：交易 → 购买 → VIP MONTHLY PASS 横幅
- 用户裁决：两项权益整体纵向排列，每行为「左侧图标、右侧文字」；上方显示金色礼盒和「每日免费盲盒」，下方显示橙色切面宝石和「每日 100 Gems」；每行保留独立浅金色描边圆角标签。
