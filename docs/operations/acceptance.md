...
- 资产业务：开盒、转盘、进化、分解、远征、Battle、任务、邀请和图鉴奖励的并发扣减与重复提交；开盒当前运行期断网只查询原 `operation_id`，首次取得成功终态后按服务端 `order` 播放同一套连续揭晓，十连在结果及时返回时从开场到汇总约 7 秒，普通结果快于高稀有度结果。逐件记录中央主图、稀有度光效、原创音效与关键节点轻微触觉反馈；右上角“跳过”在结果已到时直接进入汇总、结果未到时只登记本地意图。最终十连轮播仍按服务端原始顺序排列并依次入场，结果按钮最后入场；系统减少动态效果、设备静音、不支持 Web Audio 或 Telegram 触觉时静默降级。服务端终态弹窗的“确定”和“去藏品查看”网络记录中不得出现结果确认请求，“再开一次”只能提交一个新的 `gacha.open`。开盒结果出现后隐藏、刷新、关闭或重进必须直接显示开盒首页和最新权威状态，不恢复旧结果。转盘继续在刷新、关闭、断网和重进后恢复原完整结果，服务端回执完成前持续出现并锁定领域按钮与底部导航；结果逐项核对有序奖励、固定扣款、K-coin 奖励返入、净变化、里程碑、免费资格、今日次数和最终资产；覆盖十连扣 180、返入 140、净 -40，以及 9 次单转合计扣 180、返入 100、净 -80，均不得误报为少扣款。
