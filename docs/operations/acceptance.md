...
- 开盒代码特效与结果图片同步：在同一开发 deployment 的真实 iPhone Telegram 与 Safari Web Inspector 中分别以冷缓存执行单抽和十连。首次点击后确认只提交一个 `gacha.open`，仪式 DOM 同时存在一个静态场景图、内联 SVG 和 Canvas，网络瀑布不得出现用于光环、裂纹、碎片、闪电、粒子或逐帧切换的新增效果图片、音频文件或第二次业务请求。前 3.3 秒只出现暖白与暖金的水波、三次脉冲、光环、晶格、裂纹和中性触觉，不得从盲盒档次或提前返回的结果泄露稀有度；服务端终态与单抽详情图或全部十连缩略图均完成 `decode()` 后，最后 0.7 秒才按单抽真实稀有度或十连最高真实稀有度改变 CSS 光爆、SVG 闪电、Canvas 粒子、合成音和冲击反馈。普通快速路径从动画组件挂载到完整结果首次可见必须为 4 秒；人为延迟原 operation 或任一结果图片超过 3.3 秒时，画面必须冻结在无稀有度临界裂纹状态，不循环、不继续运动、不显露结果标题、按钮、NEW、空图片区域或剪影，准备完成后只播放一次 0.7 秒揭晓，原图片节点随完整结果一次性显现且不重新读取。四核及以下设备验证 Canvas 像素比和粒子上限降级，系统减少动态效果验证静态临界舞台及相同门控。再使任一结果图片连续经过首次读取、1 秒和 3 秒重试后最终失败，中性收束后画面必须停在开启后的月夜庭院并只显示“灵契尚未显现”和“再试一次”；点击重试后网络记录只能出现同一批公开 `pet-runtime` 图片读取，不得出现新的 `gacha.open`、operation 查询或确认请求，K-coin、免费资格、保底、藏品、图鉴、任务和 operation 数量保持失败前权威值。恢复图片读取后完整结果整体显现；只有随后点击结果舞台的“再开一次”才允许创建一个新 `gacha.open` 并产生对应的新业务结算。
