...
- 开盒代码特效与结果图片同步：在同一开发 deployment 的真实 iPhone Telegram 与 Safari Web Inspector 中分别以冷缓存执行单抽和十连。默认开盒页首屏事实就绪后，开盒表现 chunk 必须已通过动态边界准备；首次点击不得出现浅色通用处理页，且只提交一个 `gacha.open`。结果舞台点击“再开一次”后，新 `operation_id` 必须复用当前内存中已加载且仍与 `gacha.open` 匹配的表现模块，第一帧直接进入中性黑洞舞台，不得回退到浅色通用处理页。演出 DOM 同时存在一个程序化深黑金色背景和 Canvas，Canvas 必须取得 WebGL2 上下文且网络瀑布不得出现用于黑洞、吸积环、粒子、云尘、冲击环或逐帧切换的新增效果图片、粒子贴图、视频、音频文件、第三方特效运行时或第二次业务请求。召唤、图片失败重试和结果页必须始终挂载同一个 `GachaAstralBackdrop`，截图与计算样式证明三阶段的深黑暖金颜色、黑洞中心位置和空间层级相同，网络瀑布不得请求开盒背景图片，结果交接前不得改变舞台结构。前 4 秒只出现从小到大且严格完成 13 次呼吸的深黑核心，完整周期必须依次为 `800 / 580 / 460 / 380 / 330 / 290 / 260 / 230 / 200 / 170 / 130 / 100 / 70ms`；连续取帧必须证明每次先变大再变小、每轮低点和高点均高于上一轮，并在最后 `470ms` 完成四次只作用于黑洞尺寸与事件视界光环的极速临界频闪。暖金吸积环、沿弧线旋转并收紧半径的香槟金粒子、三次中性触觉仍与该舞台同步；粒子连续帧的方位角与半径必须同时变化，不能呈直线径向吸入，中央不得出现蛋、球、胶囊、水晶或其他实体容器，也不得从盲盒档次或提前返回的结果泄露稀有度。服务端终态与单抽详情图或全部十连缩略图均完成 `decode()` 后，最后 0.7 秒才把少量真实稀有度色调混入金色主体，并显示 WebGL 冲击环、CSS 全屏金光、放射光线、合成音和冲击反馈。普通快速路径从动画组件挂载到完整结果首次可见必须为 4.7 秒；人为延迟原 operation 或任一结果图片超过 4 秒时，画面必须冻结在无稀有度的最大黑洞临界状态，不重新呼吸、不继续吸入、不显露结果标题、按钮、NEW、空图片区域或剪影，准备完成后只播放一次 0.7 秒揭晓，原图片节点随完整结果一次性显现且不重新读取。普通设备检查 `data-astral-quality=standard`、`520` 条粒子与 `1.25` 像素比上限；四核及以下设备检查 `data-astral-quality=low-power`、`320` 条粒子与 `1` 像素比上限；系统减少动态效果检查 `96` 个静态金色粒子、最大黑洞静态临界舞台及相同门控。Canvas 必须同时暴露 `data-astral-breath-count=13` 和与上述周期一致的 `data-astral-breath-periods`，中性汇聚切换到揭晓时同一 Canvas 和 WebGL context 必须保持，粒子相位连续，不能重新编译 program 或重建 buffer。Safari Web Inspector 以 Canvas 存续期 `requestAnimationFrame` 采样记录帧数、平均/最大帧间隔及超过 25、34、50 毫秒的帧数，并与修改前基线对照；阻断 WebGL2 时检查 Canvas 2D 降级的普通、四核及以下和减少动态效果档分别使用 `180 / 120 / 48` 条螺旋粒子或静态粒子。再使任一结果图片连续经过首次读取、1 秒和 3 秒重试后最终失败，中性金光收束后画面必须停在静止深黑金色灵契舞台并只显示“灵契尚未显现”和“再试一次”；点击重试后网络记录只能出现同一批公开 `pet-runtime` 图片读取，不得出现新的 `gacha.open`、operation 查询或确认请求，K-coin、免费资格、保底、藏品、图鉴、任务和 operation 数量保持失败前权威值。恢复图片读取后完整结果整体显现；只有随后点击结果舞台的“再开一次”才允许创建一个新 `gacha.open` 并产生对应的新业务结算。
