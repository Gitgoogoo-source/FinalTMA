...
- 全局顶部资产栏：五个主页只出现同一个资产栏；Fgems 固定显示绿色切面宝石图片，K-coin 固定显示紫色星星图片且星星轮廓、朝向和比例与已确认参考图一致，两枚 `64px × 64px` 透明 PNG 均按 `20px × 20px` 显示，按钮内只保留图标与数值；两项余额数值均使用 Teko SemiBold `600` 窄体粗数字并显示完整十进制整数，不出现逗号、空格、其他千位分隔符或 `K`、`M` 缩写；分别记录 Telegram iOS、Android、Desktop 与 Web 在原生全屏成功和不支持回退时的截图，确认设备安全区、内容安全区、视口与方向变化后，返回、关闭和更多控件始终位于资产栏上方且不重叠。
