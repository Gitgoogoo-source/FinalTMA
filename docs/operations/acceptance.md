...
- 首屏：生产构建打印的闭包必须同时满足 JS `470000 / gzip 135000` 字节、CSS `110000 / gzip 23000` 字节，禁止模块为零且无 Vite 大 chunk 警告；该静态结果不能替代真机验收。真实 Telegram 首次登录与默认开盒页必须确认入口、开盒页和首屏契约先加载，市场、Battle、藏品、任务、转盘、图鉴、充值/VIP 弹窗和六类结果 JS/CSS 在对应意图前不请求；`identity.bootstrap` 整体失败保留内存会话，VIP 和默认开盒摘要分别失败时只关闭自身入口并可局部重试，网络记录不得出现钱包摘要请求。
