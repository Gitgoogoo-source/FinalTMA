...
- 资产业务：开盒、转盘、进化、分解、远征、Battle、任务、邀请和图鉴奖励的并发扣减与重复提交；开盒当前运行期断网只查询原 `operation_id`，服务端终态弹窗的“确定”和“去藏品查看”网络记录中不得出现结果确认请求，“再开一次”只能提交一个新的 `gacha.open`。开盒结果出现后隐藏、刷新、关闭或重进必须直接显示开盒首页和最新权威状态，不恢复旧结果。转盘继续在刷新、关闭、断网和重进后恢复原完整结果，服务端回执完成前持续出现并锁定领域按钮与底部导航；结果逐项核对有序奖励、固定扣款、K-coin 奖励返入、净变化、里程碑、免费资格、今日次数和最终资产；覆盖十连扣 180、返入 140、净 -40，以及 9 次单转合计扣 180、返入 100、净 -80，均不得误报为少扣款。
