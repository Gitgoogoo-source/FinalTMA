...
- 首屏：生产构建打印的闭包必须同时满足 JS `400000 / gzip 125000` 字节、CSS `110000 / gzip 23000` 字节，禁止模块为零且无 Vite 大 chunk 警告；仓库内不得存在共享 UI barrel、其导入或 React Router 依赖。入口在 React 渲染前只能预取已登记的首屏契约，重型 `OperationRegistryRuntimeProvider` 不得进入闭包。该静态结果不能替代真机验收。真实 Telegram 首次登录与默认开盒页必须确认入口、开盒页和首屏契约先加载，认证响应携带的 `initial_state.summary` 写入 `identity.summary` 缓存，`recovery` 只写入当前 session generation 内存；默认开盒页的数据、规则、身份摘要和当前盲盒主图全部就绪后，网络允许动态准备开盒操作 Runtime、开盒表现 JS/CSS 和唯一月夜底图，但不得请求业务 API、结果宠物图或其他领域表现，且这些资源不得进入首屏同步闭包。市场、Battle、藏品、任务、转盘、图鉴、充值/VIP 弹窗和其余五类结果 JS/CSS 在对应意图或真实恢复需要前不请求；第一次操作必须立即反馈且只提交一次，开盒首次点击不得出现浅色通用处理页，运行时加载失败不得发送业务 API。`identity.initial` 整体失败保留内存会话并可重试，VIP 和默认开盒摘要分别失败时只关闭自身入口并可局部重试，网络记录不得出现钱包摘要请求。
...
- 开盒代码特效与结果图片同步：在同一开发 deployment 的真实 iPhone Telegram 与 Safari Web Inspector 中分别以冷缓存执行单抽和十连。默认开盒页首屏事实就绪后，开盒表现 chunk 与唯一月夜底图必须已通过动态边界准备；首次点击不得出现浅色通用处理页，且只提交一个 `gacha.open`。仪式 DOM 同时存在一个静态场景图和 Canvas，Canvas 必须取得 WebGL2 上下文且网络瀑布不得出现用于灵绸、光核、光尘、碎片、水面尾迹、冲击波或逐帧切换的新增效果图片、粒子贴图、视频、音频文件、第三方特效运行时或第二次业务请求。仪式、图片失败重试和结果页必须始终引用同一个 `moonlit-prism-garden-b1291c69.webp`，截图与计算样式证明三阶段的 `object-position`、亮度和缩放相同，中央透明衬底仍能看见同一水面、月牙和晶翼，结果交接前不得切图、放大、降亮或整屏闪色。前 3.3 秒只出现暖白与暖金的六股同向旋转、非对称 S 形纤维灵绸、三次脉冲、光核、流场光尘、星芒、菱形碎片、水面螺旋尾迹和中性触觉；灵绸不得形成对称沙漏、蝴蝶、三个封闭弯刀、叶片或其他实体外轮廓，中央不得出现蛋、球、胶囊、水晶或其他实体容器，也不得从盲盒档次或提前返回的结果泄露稀有度。服务端终态与单抽详情图或全部十连缩略图均完成 `decode()` 后，最后 0.7 秒才按单抽真实稀有度或十连最高真实稀有度改变 WebGL 灵绸纤维、粒子爆散距离、冲击波、CSS 光爆、合成音和冲击反馈。普通快速路径从动画组件挂载到完整结果首次可见必须为 4 秒；人为延迟原 operation 或任一结果图片超过 3.3 秒时，画面必须冻结在无稀有度灵绸紧束临界状态，不循环、不继续运动、不显露结果标题、按钮、NEW、空图片区域或剪影，准备完成后只播放一次 0.7 秒揭晓，原图片节点随完整结果一次性显现且不重新读取。普通设备检查 `data-spirit-quality=standard`、`720` 粒子、`72` 段灵绸与 `1.25` 像素比上限；四核及以下设备检查 `data-spirit-quality=low-power`、`420` 粒子、`56` 段灵绸与 `1` 像素比上限；系统减少动态效果检查 `120` 个静态光尘、静态临界舞台及相同门控。蓄能切换到揭晓时同一 Canvas 和 WebGL context 必须保持，粒子相位连续，不能重新编译 program 或重建 buffer。Safari Web Inspector 以 Canvas 存续期 `requestAnimationFrame` 采样记录帧数、平均/最大帧间隔及超过 25、34、50 毫秒的帧数，并与修改前基线对照；阻断 WebGL2 时检查 Canvas 2D 降级的普通、四核及以下和减少动态效果档分别使用 `200 / 144 / 48` 个粒子且没有恢复三个实体叶片。再使任一结果图片连续经过首次读取、1 秒和 3 秒重试后最终失败，中性收束后画面必须停在开启后的月夜庭院并只显示“灵契尚未显现”和“再试一次”；点击重试后网络记录只能出现同一批公开 `pet-runtime` 图片读取，不得出现新的 `gacha.open`、operation 查询或确认请求，K-coin、免费资格、保底、藏品、图鉴、任务和 operation 数量保持失败前权威值。恢复图片读取后完整结果整体显现；只有随后点击结果舞台的“再开一次”才允许创建一个新 `gacha.open` 并产生对应的新业务结算。
