...
12. 详情打开期间连续观察人物和宠物位置不变化；关闭详情后从相同镜头和位置继续。离开游戏页再返回时允许重新生成漫游位置，但不得写入浏览器或服务端存档。
...
16. Telegram iOS、Android、Desktop 与 Web 分别记录游戏页直接首屏、顶部与底部固定栏安全区、触摸/鼠标寻路、WASD 单次与按住逐格移动、镜头跟随、宠物点击、详情暂停/恢复及离开后重新进入。普通浏览器验证不能替代 Telegram 真机结果。
