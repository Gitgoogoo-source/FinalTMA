...
- Telegram 聊天列表授权：使用同一部署 SHA 在真实 iPhone Telegram 与 Android Telegram 覆盖英语新账号、简体中文账号、既有未授权账号、已授权账号、拒绝后同一 WebView、拒绝后完整关闭重进、拒绝后再次进入并同意、多设备并发和 Telegram 重复 update。同意前必须在认证、入口交接和首个可操作页面完成后才出现一次官方原生弹窗；拒绝后当前 WebView 不重复且游戏继续可用，下次新 WebView 再次自动弹出；同意后聊天列表出现 EvoMyPet，按账号语言只收到一条固定欢迎消息和 named Mini App 按钮；已授权账号不再弹出。数据库必须只有一条 `operations.telegram_chat_onboarding`，重复 update、不同 update 和多设备都返回不可发送；`sent/failed/unknown` 均不重放。伪造 secret、非私聊、`from.id != chat.id`、`from_request` 缺失、未知账号、封禁账号和非安全整数均不得发送。旧客户端、方法抛错、数据库失败和 Telegram API 失败不得阻塞游戏；Safari Web Inspector 只检查调用时机、异常与无新增浏览器 API/Supabase 请求，不能替代原生弹窗、Bot 私聊或聊天列表结论。
