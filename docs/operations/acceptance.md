...
9. iOS 与 Android 点按可通行地面后，默认人物通过四方向寻路逐格移动；静态障碍、宠物占用格或预定格不会被穿过，下一格动态被占用时重新寻路。
10. 镜头始终固定 `0.5` 倍并平滑跟随人物；地图、人物与宠物相对原 `1` 倍镜头以 `50%` 尺寸显示，横向和纵向可视范围各为原来的 `2` 倍。地图不能拖动、滚轮或双指缩放；桌面键盘、WASD、方向键和鼠标点地不能移动人物，页面不显示摇杆、方向按钮、A/B、宠物名单、重复数量、血条、队伍或战斗 HUD。
11. 点击任一宠物后场景立即暂停且人物不移动，游戏上方显示当前认证结果匹配的现有藏品详情：正式名称、详情图、稀有度、阶段、战斗力与现有数量摘要；不显示进化、分解、出售、Mint 或藏品切换。
12. 详情打开期间连续观察人物和宠物位置不变化；关闭详情后从相同镜头和位置继续。关闭整个家园再打开时允许重新生成漫游位置，但不得写入浏览器或服务端存档。
...
16. Telegram iOS、Android、Desktop 与 Web 分别记录全屏、安全区、触摸寻路或桌面禁用移动、镜头跟随、宠物点击、详情暂停/恢复和返回游戏页。普通浏览器验证不能替代 Telegram 真机结果。
