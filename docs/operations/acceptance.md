...
1. normal 用户进入 `/game` 后立即看到占满顶部资产栏与底部导航之间可用视口的水上家园；页面不存在游戏中心标题、Monster Tamer 启动卡、“进入家园”、藏品远征或幸运转盘。任务页的幸运转盘严格位于七日签到下方、筛选栏上方；任务页不存在“远征”筛选、三张远征任务卡或远征重点横幅。
...
10. Desktop 与 Web 的 `W/A/S/D` 分别逐格控制上、左、下、右；单次按键只移动一格，按住时连续逐格移动，移动中只缓存落地后的下一格。障碍或宠物占用/预定目标格时停止，不穿墙、不跳格；方向键不移动人物。镜头始终固定 `0.5` 倍并平滑跟随，地图不能拖动、滚轮或双指缩放；页面不显示摇杆、方向按钮、A/B、宠物名单、重复数量、血条、队伍或战斗 HUD。
...
13. 分别验证加载、接口失败、无可用藏品、缩略图失败、点击模板已失效和 Phaser 运行错误；任何失败都不注入默认宠物、不沿用旧数据、不伪造详情。
