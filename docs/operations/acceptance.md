...
- 主导航页面保活：交易、游戏、开盒、藏品和任务分别首次进入一次，等待超过 React Query 的 20 秒 `staleTime` 后连续切换并返回；网络记录不得新增对应页面 API、页面模块、图片、Phaser、地图或场景资源请求，页签、筛选、选择、滚动位置和未提交界面状态保持。图鉴与 Mint 返回时恢复原主页面；顶部人工刷新只请求 `identity.bootstrap`、`vip.get`、`wallet.get`。后台不足五分钟恢复时不请求，达到五分钟时只静默刷新顶部摘要与当前路由查询。Session generation 改变或封禁后旧页面、缓存和迟到结果不得恢复。
...
1. normal 用户首次进入 TMA 时，默认开盒页完成前的网络请求不包含其他页面模块、Monster Tamer 或 TON/钱包资源；首屏完成后后台先预加载游戏页模块、`inventory.list`、Phaser 与地图/藏品资源，再预加载其他普通主导航页面。无论预加载是否完成，进入 `/game` 都立即保留顶部资产栏与底部导航并显示水面背景，页面不出现“请从 FinalTMA 游戏中心进入”、居中加载卡、全局页面加载文案、游戏中心标题、Monster Tamer 启动卡、“进入家园”、藏品远征或幸运转盘。
2. 记录 `inventory.list` 响应与地图实体清单：只显示 `available > 0` 的不同 `template_id`，同模板拥有 1 只或 20 只都只生成一个实体，`available = 0` 不生成，不存在总数截断或用户选择名单。
3. 直接访问 `/monster-tamer/` 固定显示“请从 FinalTMA 游戏中心进入”，不请求 `/api/*`、不获得会话或藏品；`/game` 中的 `/monster-tamer/?embedded=1` 从 HTML 第一帧隐藏该门禁，等待父页面消息时只显示水面背景。登录父页面只通过同源消息注入 `templateId`、名称和正式缩略图路径。
...
6. 静态树只恢复 AxulArt 默认玩家精灵及许可证，不存在旧玩家控制器、NPC、Sign、Item、Encounter、战斗、捕捉、队伍、背包、道具、菜单、旧怪物 JSON、旧图片和音频；Web Font Loader 与 Tweakpane 不再发布。
7. 全部宠物图片请求均位于 `/assets/catalog/v1/thumb/`。每只宠物主体世界尺寸严格为 `56×56` 像素，在固定 `0.5` 倍镜头下显示为改动前的 `50%`；阴影、点击区域、上下浮动和压缩伸展位移同步缩小 `50%`。不存在旧 Monster Tamer 怪物或新生成方向帧；移动中能观察到翻转、浮动、轻微压缩伸展和正式 Tiny Swords 阴影。
8. 同时观察全部不同藏品：宠物只能逐格进入相邻可通行格，不穿过水体或静态障碍，不与另一只宠物占用或预定同一格。
9. iOS 与 Android 触摸点按、Desktop 与 Web 鼠标左键点按可通行地面后，默认人物通过同一四方向寻路逐格移动；静态障碍、宠物占用格或预定格不会被穿过，下一格动态被占用时重新寻路。
10. Desktop 与 Web 的 `W/A/S/D` 分别逐格控制上、左、下、右；单次按键只移动一格，按住时连续逐格移动，移动中只缓存落地后的下一格。分别持续按住单一方向，并连续验证 `A→S`、`S→D`、`D→W`、`W→A`：人物完成当前格后，下一格立即采用最后按下且仍保持的方向，格子之间没有静止帧或等待系统键盘重复的间隔；松开后不发生持续失控移动。障碍或宠物占用/预定目标格时停止，不穿墙、不跳格；方向键不移动人物。镜头始终固定 `0.5` 倍并平滑跟随，地图不能拖动、滚轮或双指缩放；页面不显示摇杆、方向按钮、A/B、宠物名单、重复数量、血条、队伍或战斗 HUD。
11. 点击任一宠物后场景立即暂停且人物不移动，游戏上方显示当前认证结果匹配的现有藏品详情：正式名称、详情图、稀有度、阶段、战斗力与现有数量摘要；不显示进化、分解、出售、Mint 或藏品切换。
12. 详情打开期间连续观察人物和宠物位置不变化；关闭详情后从相同镜头和位置继续。离开游戏页或打开详情路由时 iframe 保持挂载、Phaser 场景与主循环暂停且持续按键被清除；返回游戏页后同一镜头、人物和宠物位置继续，网络中不重新请求 Phaser、地图或场景资源。只有真实可展示模板或正式缩略图路径变化时才允许受控重建，且不得写入浏览器或服务端存档。
13. 分别验证预加载未开始、预加载进行中、藏品查询进行中、接口失败、无可用藏品、缩略图失败、点击模板已失效和 Phaser 运行错误；前三种等待态只显示水面背景且没有居中弹窗，真实失败仍提供错误与重新加载入口。任何状态都不注入默认宠物、不沿用旧数据、不伪造详情。
14. 网络记录中只有主应用的认证 `inventory.list` 与同源静态资源；Phaser 不直接请求 API、Supabase、第三方资源或业务写接口，也不接收 access token、session generation 或 Telegram `initData`。
15. Phaser 3.60.0、Tiny Swords 31 个运行文件、32 个源文件记录、AxulArt 默认人物与许可证、Catalog 图片、MIT/Phaser 许可证、Pixel Frog 来源与条款快照均在大小写敏感部署返回 200。
16. Telegram iOS、Android、Desktop 与 Web 分别记录游戏页直接首屏、顶部与底部固定栏安全区、触摸/鼠标寻路、WASD 单次与按住逐格移动、镜头跟随、宠物点击、详情暂停/恢复及切换页面后的同场景恢复。普通浏览器验证不能替代 Telegram 真机结果。
