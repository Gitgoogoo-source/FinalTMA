...
宠物美术可独立于 Git/Vercel 回滚。操作员先用资源工具下载并校验目标历史批次全部 420 个公开对象，建立覆盖操作窗口的回滚锁，再调用单一 RPC 原子切换；任一对象缺失、SHA-256 不符、已删除或批次不完整都必须停止。公开对象已被 90 天清理任务删除时，不得把指针切向残缺批次，必须从 `art-masters` 私有历史母版重新生成并以新发布键发布完整批次。任何美术回滚都不删除或覆盖私有母版。
