...
Monster Tamer 回滚必须选择同时包含 React 家园、`inventory.list` 只读适配、`/monster-tamer` 渲染路由、50×50 地图和现有详情组件兼容关系的完整 commit，禁止只回滚父页面或只回滚静态渲染树。回滚不执行数据库、账本、库存或 Catalog 写入；部署后重新验证登录门禁、可用模板去重、同源消息、宠物漫游、详情暂停恢复、资源 200 和业务零写入。
