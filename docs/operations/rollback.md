...
Monster Tamer 回滚必须选择同时包含 `/monster-tamer`、`/monster-tamer/` 路由、直接进入世界和无浏览器持久化契约的完整静态资源树兼容 commit，禁止只回滚 launcher 或只回滚部分资源。回滚不执行数据库、API、账本、库存或 Catalog 操作，也不得清除用户浏览器中的旧 `MONSTER_TAMER_DATA`；部署后重新验证独立入口、刷新恢复初始状态、完整玩法、资源 200、返回 `/game` 和业务零请求。
