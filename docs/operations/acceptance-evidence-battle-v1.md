...
> 2026-08-04 方案替代说明：ADR-026 已删除 Battle `current_result`、acknowledge、确认时间和跨会话结果恢复。本文下方涉及这些机制的内容只保留为对应历史版本的真实证据，不代表当前产品方案或当前验收标准。
...
Battle 05 的两条脱敏现场事实继续保留：

1. 一次真实场景中，倒计时启动约 1.776 秒后 B 离页；离线事件送达后，旧实现中止倒计时，重进时从完整 3 秒重新开始。
2. 另一次真实场景中，A 在倒计时 2 秒内离页；离线事件未送达，旧实现保留截止时间并在约 3.262 秒后唯一开战。

独立实现审查确认分叉来自数据库同一旧状态机：`battle.reconcile_lobby_presence` 收到有效离线或 heartbeat 超时后把 `lobby_countdown` 回退为 `lobby_waiting` 并清空截止时间；未收到离线事件时，原截止时间保留并由 tick 推进开战。该差异不是 Telegram 宿主关闭能力的产品选择，而是与最新唯一规则冲突的实现缺陷。

R11 只记录文档同步、实现修复、静态门禁、数据库从零重建、远端定义读回和空状态/自然 tick 证据，不执行也不宣称完整真实 Telegram 回归。以下影响域保持 `UNVERIFIED`，由全新 Battle V13 任务独立执行：真机全屏倒计时专页及所有按钮遮挡；倒计时 1 秒与 2 秒位置分别触发可送达 offline 和不可送达 offline；旧/新 lease、乱序 command seq、页面刷新、重新认证和关闭后重进；同一截止时间 exactly-once 产生唯一 `battle_started` event、Outbox 与 turn 1；全程无提前退款、重复锁定、重复 reservation 或重复结算。
