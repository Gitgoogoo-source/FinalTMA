...
| Monster Tamer | React 家园 + 同源 Phaser 渲染树 | 与生产相同提交的认证只读家园与渲染树                                   | 与开发验收相同的家园与渲染树 |
...
Monster Tamer 在三个环境中均由同一 Git commit 内的 React 领域组件和 `apps/web/public/monster-tamer` 渲染树共同提供。React 使用现有 FinalTMA 内存会话调用 `inventory.list`，Phaser 不接收会话或请求 API。家园不新增环境变量、数据库对象、支付或链上配置，不使用浏览器持久化；宠物漫游状态不在环境间迁移或同步。
