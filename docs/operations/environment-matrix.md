...
Battle 发布后，真实开发固定启用 Web、API、Supabase、Telegram webhook、Ably subscribe-only、`battle-tick-v1`、两个 Battle integrations、支付对账、幂等清理和不变量监控。TON 配置为空，`reconcile-mints` 不进入 Vercel Cron；钱包验签、Mint permit 和 Mint 对账只有在完成 testnet Collection 部署并写入全部真实 TON 配置后才启用。非 TON API 只解析自身所需配置，不接受任何 TON 占位值。
