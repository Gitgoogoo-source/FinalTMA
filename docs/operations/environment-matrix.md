...
| Monster Tamer | React/Phaser 与本地 Postgres 17 | 与生产相同提交的 API、Schema、玩法资料和目录图片                       | 与开发验收相同的完整运行链   |
...
Monster Tamer 在三个环境中均使用同一 Git commit 的 React/Phaser bundle、`@pokepets/api-contracts/app`、三个 `monster_tamer` RPC、固定世界/技能资料和正式目录图片。它复用当前 TMA 内存会话，进度只保存到各环境的 `monster_tamer` schema，不使用浏览器持久存储；开发与生产数据库彼此隔离，不迁移玩家进度。
