...
- Monster Tamer 的 React/Phaser bundle、三条 API 契约、声明式 Schema、固定世界/战斗资料和 210 张目录图片路径来自同一提交；旧 `apps/web/public/monster-tamer` 整包已经删除，不存在旧公开 HTML、本地存档或捕捉/经验运行入口。
...
`pnpm architecture:check` 同时验证 Monster Tamer 的登录内覆盖层、游戏页顺序、Phaser bridge 隔离、三个服务端路由、数据库所有权、无浏览器存档、旧公开入口不可达和 SPA 路由顺序。`APP_ENV=development pnpm build` 后必须确认 Phaser 延迟 chunk 和目录图片均可从构建产物读取。
...
5. 对开发项目执行 `supabase db lint --linked --schema api,identity,catalog,operations,economy,inventory,gacha,expedition,wheel,market,payments,vip,tasks,referral,album,monster_tamer,onchain,risk --level warning --fail-on error` 并运行 Supabase security/performance advisors。
...
9. 对当前 Vercel 部署请求 `/monster-tamer` 与 `/monster-tamer/`，确认两者进入主应用账号门禁而非独立游戏；从开发 Bot 登录后按 `docs/operations/acceptance.md` 完成真实藏品、八区探索、服务端战斗、跨设备存档、键盘/触控和资产零副作用验收。
...
9. 对生产域名执行 `/monster-tamer`、`/monster-tamer/`、游戏覆盖层、返回和目录资源 smoke check，确认未登录不能进入，登录后使用同一提交中的 API、Schema、固定玩法资料和 Phaser bundle。
