...
- Monster Tamer 的 `apps/web/public/monster-tamer` 静态树完整，固定上游提交、MIT 许可证、第三方声明和资源来源记录均已提交；所有没有直接授权证据的上游图片已经替换为项目原创图片，且完整玩法未被删减。
...
`pnpm architecture:check` 同时验证 Monster Tamer 静态入口、launcher 纯链接、游戏页顺序、独立本地存档、业务引用为零和 Vercel 路由优先级。`APP_ENV=development pnpm build` 后必须确认 `apps/web/dist/monster-tamer` 与源静态树文件清单一致；缺失任一脚本、样式、字体、音频、地图、数据、图片、许可证或第三方声明时停止发布。
...
9. 对当前 Vercel 部署分别请求 `/monster-tamer` 与 `/monster-tamer/`，确认两者返回独立游戏文档且全部静态资源成功；按 `docs/operations/acceptance.md` 完成 Monster Tamer 的完整玩法、键盘、触控、本地存档、业务零请求和图片替换验收。
...
9. 对生产域名执行 `/monster-tamer`、`/monster-tamer/`、游戏内返回和静态资源 smoke check，确认其使用同一提交中的完整静态树且没有新增环境变量、API、数据库或 Catalog 依赖。
