...
- 按钮音效：文字、图标、底部导航、页签、弹窗与按钮式卡片的可信可用按钮点击统一播放用户提供的同一份短 MP3；禁用控件和普通游戏画面点击静默，音效失败不阻塞按钮动作，完整边界见 [ADR-091](adr/ADR-091-global-button-click-audio.md)。
...
- [全局统一按钮点击音效](adr/ADR-091-global-button-click-audio.md)
