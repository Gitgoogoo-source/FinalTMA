...
默认开盒表现动态边界在页面空闲时完成 Canvas/WebGL 程序资源和首帧准备；五核及以上设备另以有界空闲任务准备 Web Audio 风声样本，四核及以下设备固定不创建或播放自定义 Web Audio。离屏 Canvas 不预分配全屏 surface，首次可见呼吸不得承担 GPU 程序资源冷创建、整段音频样本同步生成或低功耗设备音频管线建立，唯一规则见 [ADR-069](adr/ADR-069-gacha-renderer-prewarm-and-static-stage.md)。
