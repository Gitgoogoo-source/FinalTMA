...
默认开盒表现动态边界在页面空闲时通过永久文档根层 surface 完成实际视口 drawing buffer、Canvas/WebGL 程序资源和首帧准备；WebGL 首帧必须以 `finish()` 在该空闲任务内同步完成，不能把仍在驱动队列中的绘制标为已预热。五核及以上设备另以有界空闲任务准备 Web Audio 风声样本，四核及以下设备固定不创建或播放自定义 Web Audio，也不触发演出期 Telegram `HapticFeedback`。首次可见呼吸不得承担 GPU 程序资源冷创建、drawing buffer 首次扩展、延迟 GPU 提交、整段音频样本同步生成、低功耗设备音频管线建立或原生触觉桥接，唯一规则见 [ADR-069](adr/ADR-069-gacha-renderer-prewarm-and-static-stage.md)。
