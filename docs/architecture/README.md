...
默认开盒表现动态边界在页面空闲时按当前 WebView 全屏尺寸完成 Canvas drawing buffer 和首帧准备，并分块生成 Web Audio 风声样本；首次可见呼吸不得承担 `1 × 1` 画布扩容或整段音频样本同步生成，唯一规则见 [ADR-069](adr/ADR-069-gacha-renderer-prewarm-and-static-stage.md)。
