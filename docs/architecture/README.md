...
默认开盒表现动态边界在页面空闲时完成 Canvas/WebGL 程序资源、首帧和有界 Web Audio 风声样本准备；离屏 Canvas 不预分配全屏 surface，首次可见呼吸不得承担 GPU 程序资源冷创建或整段音频样本同步生成，唯一规则见 [ADR-069](adr/ADR-069-gacha-renderer-prewarm-and-static-stage.md)。
