...
Web 只导入 `@pokepets/api-contracts/app`。领域 UI 不导入其他领域，库存页组合库存、进化和分解，任务页组合任务和邀请，市场页组合市场和 VIP。首次进入 TMA 时同步下载并执行应用壳、会话与账号门禁及默认开盒页；`window.load` 后在浏览器空闲时段预加载交易、藏品、任务和图鉴页面模块。游戏页保留为空页面，不发起业务查询或资源预加载；Mint 页面、TON Provider 与钱包弹窗不进入后台预加载。
...
Web 与三个 Functions 位于同一 Vercel Pro Project，Functions 运行时为 Node.js 24。版本化藏品静态资源使用一年 immutable 缓存，已发布目录不可覆盖；`contracts/ton` 使用独立 `pnpm chain:build` 门禁。真实开发环境与未来生产环境使用同一 Git commit 和迁移序列。
