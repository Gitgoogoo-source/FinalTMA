...
正式宠物母版永久保存在 Supabase Storage 私有桶 `art-masters`，公开桶 `pet-runtime` 只保存 420 张当前运行时 WebP；Git 只保存资源清单、SHA-256 和发布工具。列表直接读取 API 返回的 256×256 公开桶完整 URL，主视觉读取 768×768 公开桶完整 URL；这是浏览器唯一允许的 Supabase 直连，浏览器不加载 Supabase SDK、不持有 key，也不连接 Postgres、RPC、Auth 或其他 Data API。读取失败时保持布局并显示 Vercel 内置统一宠物剪影，后台有限重试且不显示后端相关文案。进化静态路线只保存模板身份与规则，运行时图片统一从当前目录映射解析。
