...
| 21 Monster Tamer | monster-tamer                   | monster-tamer                            | monster_tamer、catalog、inventory、operations | 登录内全屏、真实可用藏品、跨设备进度、逐回合裁决    |
...
- Monster Tamer 的 Phaser 运行时不得接收会话或 API 能力；认证 API 只读目录与可用藏品，写操作只能改变 `monster_tamer` 进度和战斗，不能改变任何 TMA 资产。
