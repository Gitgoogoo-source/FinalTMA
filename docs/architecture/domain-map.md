...
| 11 分解            | inventory、decomposition                                         | decomposition                                           | 33_decomposition、inventory、economy     | 数量与 Gems 在同一事务变化                                                                                                   |
...
| 16 登录            | session-bootstrap、telegram-chat-onboarding、platform、app/shell | identity、telegram-webhook、stars-payment、battle-share | identity、operations                     | 登录与会话、首次英语、账号语言跨设备同步、原生私聊授权、聊天列表与欢迎消息至多一次、五个主页共用账号菜单、稳定 ID 内容本地化 |
...
- Telegram 写入私聊授权只在新 WebView 的首个可操作页面后请求一次；服务端只依据经 secret 校验的私聊服务消息与数据库正常账号发送，首次欢迎消息在外部调用前原子领取且不重放。
