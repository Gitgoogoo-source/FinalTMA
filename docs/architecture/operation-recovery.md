...
重新进入应用时，`identity.bootstrap.blocking_operations` 交给操作恢复工作流，`pending_payments` 和 `pending_mints` 分别交给支付与 Mint 恢复工作流。开盒和幸运转盘的 `pending`、`unknown` 以及尚未确认展示的 `succeeded`、`failed` 都进入 `blocking_operations`；前端同时通过各自的 `GET /api/gacha/recovery`、`GET /api/wheel/recovery` 按 1 秒、2 秒、3 秒、5 秒、此后每 30 秒发现事务提交晚于首屏读取的原操作，发现后停止对应发现轮询并只按原 `operation_id` 查询终态。K-coin 创建订单命令在 invoice URL 持久化后立即完成，不作为长期阻塞操作；未提交 Stars 的 `pending` K-coin 订单不恢复、不打开弹窗。只有 `processing`、`paid` K-coin 订单恢复锁定弹窗，并按 1 秒、2 秒、3 秒、此后每 5 秒查询唯一终态；VIP 继续使用自身既有恢复交互。充值订单交付后，只恢复开盒、市场购买或转盘的确认界面，不自动执行原业务。

操作弹窗先按 `use_case` 选择展示组件，再校验该命令的唯一输出 Schema。`gacha.open` 与 `wheel.spin` 只有在持久状态为 `succeeded` 且完整结果通过各自合约校验时才展示领域专用结果；组件直接呈现服务端返回的有序结果与最终业务事实，并保留通用操作号与恢复能力。未确认、失败或结果结构不完整时不展示任何成功内容；持久成功结果结构不完整时按 `unknown` 保持锁定，只查询原操作且不得确认。专用弹窗的字段和动作不在架构文档重复定义，分别唯一引用产品主文档第 14.4.6 节和第 4.3.6 节。

开盒结果关闭、再次开盒或前往藏品页之前，前端调用 `POST /api/gacha/results/:operation_id/acknowledge`；转盘结果关闭前调用 `POST /api/wheel/results/:operation_id/acknowledge`。数据库只允许当前用户按原用例确认自己的终态，并以首次确认时间幂等落库。确认响应丢失时弹窗保持打开并允许重试；确认完成后启动与领域恢复查询都不再返回该结果。前端内存和浏览器存储都不充当结果或确认事实来源。

每日幂等清理不得删除尚未确认展示的开盒或转盘终态；确认完成后，该操作才按通用终态规则在创建满 30 天后清理。
