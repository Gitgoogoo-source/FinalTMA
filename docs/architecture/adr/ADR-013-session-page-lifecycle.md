...
开盒召唤结果只属于当前前台运行期。`visibilitychange` 隐藏、Telegram `deactivated` 或 `pagehide` 立即从操作注册中心移除所有 `gacha.open` 展示与交互锁；重新可见、`activated`、`pageshow` 或重新联网时只精确刷新开盒的权威资产、资格、保底、藏品、图鉴和任务状态，不恢复旧召唤结果。刷新、WebView 重载或重新认证时 `identity.bootstrap.blocking_operations` 也不返回 `gacha.open`。全局操作恢复发现只在当前正常 session generation 的入口交接完成、页面可见、Telegram 已激活且浏览器在线时调用 `GET /api/operations/recoverable`，并只发现转盘与进化记录。`visibilitychange` 隐藏、Telegram `deactivated` 或 `offline` 会清除下一次计时并 abort 当前请求；重新可见、`activated` 或 `online` 后立即读取。发现记录时一次注入全部结果并暂停新发现，操作注册中心中的结果队列清空后立即执行一次追赶读取，再进入 1 秒、2 秒、3 秒、5 秒、此后 30 秒的空结果节奏。
