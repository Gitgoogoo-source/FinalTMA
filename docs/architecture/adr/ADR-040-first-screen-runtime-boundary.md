...
Web 首屏固定为入口、默认 `GachaPage` 和 `client-routes/first-screen` 三个根及其全部同步 import 闭包，并覆盖首屏可用前由入口无条件发起的应用 JS。Telegram 初始化完成后、React 渲染前只预取首屏契约；认证后运行时直接挂载轻量操作 Facade，完整 `OperationRegistryRuntimeProvider` 不在首屏可用前请求。默认开盒页的数据、规则、身份摘要与当前盲盒主图全部就绪并发布首屏就绪信号后，Facade 固定通过动态 import 准备 `OperationRegistryRuntimeProvider`、开盒表现 chunk 和唯一月夜底图；这些资源不计入同步闭包，不挂载结果、不调用业务 API、不读取结果宠物图。Facade 在玩家 `pointerdown`、键盘 `focus`、直接执行操作或发现真实待恢复 operation 时复用同一动态加载任务；模块就绪前先锁定重复动作，模块失败不发送业务 API，并保留重新加载入口。交易、Battle、藏品、任务、转盘、图鉴等页面继续在用户导航或受控后台策略下加载，不进入三个首屏根的同步闭包。后台页面模块预热的启动握手、网络条件、顺序、失败与玩家意图规则全部由 [ADR-043](ADR-043-adaptive-page-module-warmup.md) 接管；Battle 不再自动预热。共享 UI 直接依赖和原生浏览器导航边界由 [ADR-046](ADR-046-first-screen-direct-dependency-and-native-navigation.md) 固定。

轻量 `OperationRegistryProvider` 按 [ADR-051](ADR-051-operation-registry-selective-subscription.md) 只提供身份稳定的 Store Context、首次意图队列、会话 generation 隔离和加载失败重试；重型 `OperationRegistryRuntimeProvider` 才持有操作阶段、幂等键、原 operation 恢复、刷新范围、导航锁、前后台清理和通用处理层。Runtime controller 通过 ref 委托命令，并只向 Store 发布按 route 阻塞、导航锁、恢复队列与转盘 epoch 拆分的派生信号，不把完整 Runtime value 写回 Context。消费 `identity.initial.recovery.blocking_operations` 与统一发现结果的恢复 effect 固定通过 `useEffectEvent` 调用最新 `hydrate`，其重新执行只由 operation snapshot、会话和恢复活动条件决定；入口恢复快照只存在于 generation-scoped 内存，不注册 React Query。Runtime 发布新的展示状态不得使相同 snapshot 再次水合。首次运行时代码尚未就绪时只接受一个玩家操作，后续重复动作立即拒绝；运行时挂载后沿用既有完整并发与恢复规则。开盒、进化、分解、市场、转盘和图鉴的结果组件、图片数据及 CSS 分别由 `presentation-loader` 按 `use_case` 动态加载。按钮 `pointerdown`、键盘 `focus` 和提交入口复用缓存加载任务；表现模块与业务请求并行。表现模块失败只重载表现，不重提业务操作；运行时或契约模块在业务请求发出前加载失败时明确失败，不创建 `unknown`。开盒 3.3 秒中性蓄能与 0.7 秒揭晓从专用表现组件实际挂载后开始，完整演出、权威结果与结果图片解码缺一不可。
...
- 首屏运行时边界不改变数据库权威、产品功能或玩家最终结果；身份首屏状态与摘要读模型的契约和 migration 变更由 ADR-049 独立裁决。

## 验收

影响域必须通过格式、ESLint、Web/API/Contracts TypeScript、OpenAPI 漂移、架构检查和生产构建。架构检查必须同时禁止首屏可用前预取重型 Runtime、共享 UI barrel、React Router，以及禁止恢复 effect 把 `hydrate` 身份作为重新执行条件；默认开盒页首屏就绪后的开盒动态准备是唯一自动例外。生产构建必须打印四项首屏闭包字节并满足硬门禁，Vite 不得再出现大 chunk 警告，禁止模块数量必须为零。真实开发环境还必须在 Telegram iOS 与 Android 覆盖首次登录、默认开盒、充值/VIP、六类结果、未决/unknown 恢复、导航前进/后退和专用 chunk 阻断；首屏就绪前不得请求 `OperationRegistryRuntimeProvider`，首屏就绪后只能动态准备开盒 Runtime、开盒表现和唯一月夜底图，第一次开盒必须立即进入统一月夜舞台、只提交一次且不得闪出浅色通用处理页，真实恢复存在时运行时必须加载并接管。`pending` 与 `unknown` 重进不得因 Runtime 状态发布重复水合同一 snapshot，必须持续查询同一 ID、控制台没有 React maximum update depth 错误，并在终态后停止恢复。验证无 FOUC、无重复提交、表现失败只重载 UI，并用资源瀑布证明其他领域运行时与 JS/CSS 仍只在对应意图或真实恢复需要后请求。静态结果不得记录为真机验收通过。
