...
生产构建插件遍历三个首屏根的同步 chunk 图，对 JS 与关联 CSS 文件去重后逐文件累计原始和 gzip 字节。硬门禁固定为 JS `470000 / gzip 135000` 字节、CSS `110000 / gzip 23000` 字节；非首屏页面、非首屏 route contract、六类结果表现、完整 Battle 模型、进化目录 JSON、dormant Wallet/Mint 和 `global.css` 任一进入闭包即失败。门禁不得使用 `manualChunks`、同步 vendor chunk、提高 warning 阈值或改变统计根规避。
...
影响域必须通过格式、ESLint、Web/API/Contracts TypeScript、OpenAPI 漂移、架构检查和生产构建。生产构建必须打印四项首屏闭包字节并满足硬门禁，Vite 不得再出现大 chunk 警告，禁止模块数量必须为零。真实开发环境还必须在 Telegram iOS 与 Android 覆盖首次登录、默认开盒、充值/VIP、六类结果、未决/unknown 恢复和专用 chunk 阻断；验证即时反馈、无 FOUC、无重复提交、表现失败只重载 UI，并用资源瀑布证明领域 JS/CSS 只在对应意图后请求。静态结果不得记录为真机验收通过。
