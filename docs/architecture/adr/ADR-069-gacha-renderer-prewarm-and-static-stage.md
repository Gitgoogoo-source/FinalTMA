...
同一表现模块加载完成后还必须在空闲回调中直接创建可复用的开场 Web Audio 风声 `AudioBuffer`，并分块填充固定噪声样本；用户点击只负责解锁 `AudioContext`，动画组件挂载时只创建轻量音频节点并复用该 Buffer，不得把整段 Buffer 的创建、复制或逐样本填充放进 `AudioContext.resume()` 的异步回调或首轮可见呼吸。Buffer 尚未准备、音频能力缺失或浏览器拒绝解锁时允许省略风声层，振荡器、触觉、动画计时和业务请求继续执行，禁止回退为可见动画期间的同步生成。
