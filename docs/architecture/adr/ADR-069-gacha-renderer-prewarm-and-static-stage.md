...
同一表现模块加载完成后还必须在空闲回调中分块生成开场 Web Audio 风声所需的固定噪声样本；用户点击只负责解锁 `AudioContext` 并把已准备样本复制为一次可复用的 `AudioBuffer`，动画组件挂载时只创建轻量音频节点并复用该 Buffer，不得在首轮可见呼吸期间同步分配并逐样本填充四秒音频数组。样本尚未准备、音频能力缺失或浏览器拒绝解锁时允许省略风声层，振荡器、触觉、动画计时和业务请求继续执行，禁止回退为可见动画期间的同步生成。
