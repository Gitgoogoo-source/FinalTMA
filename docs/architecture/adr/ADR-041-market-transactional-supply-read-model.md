...
`market_create_listing` 在既有用户事务锁内通过卖家汇总行数裁决十种模板上限。购买、下架、FIFO、最终库存、reservation、余额结算、手续费、月卡返还与幂等回放仍锁定原始挂单；两级汇总绝不成为交易结算依据。
...
真实开发验收从空数据库连续应用 baseline、product data 和 security 三条迁移，覆盖上架、追加、十种限制、FIFO、部分/全部成交、下架与购买并发、幂等回放、封禁切换、SOLD 游标、失败回滚和两项不变量。性能证据在同一当前供给下增加 100,000 条终态历史，比较三个读取结果、`EXPLAIN (ANALYZE, BUFFERS)`、读写耗时、shared buffers、死锁与超时；读取计划不得访问历史挂单或无界成交聚合。
