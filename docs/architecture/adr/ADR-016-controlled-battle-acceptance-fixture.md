...
每个角色固定拥有一个 1 阶、一个 2 阶和一个 3 阶模板，并按 P01、P08、P12 档案覆盖 S01—S10；四角色合计覆盖火焰、草系、土系、雷电、水系。P01 镜像速度用于证明同速时创建者固定先手，P08/P12 用于证明不同首发速度的先手裁决，技能集合覆盖不同威力、命中率和位置；每个首模板数量 2、其余数量 1 同时覆盖单份 reservation 竞争与同模板额外可用数量。A/B/C 可承担 500 档与接受竞争，四角色均可承担 20/100 档，D 对 500 档形成余额不足事实。
