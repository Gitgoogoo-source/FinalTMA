...
`@EvoMyPet_bot` 的 Main Mini App 和默认菜单按钮可以在 BotFather 中无损关闭，但 named Mini App `evomypet` 只提供编辑 Web App URL 与删除 Web App。BotFather 不接受用 `/empty` 清空 named Mini App URL；删除会使短名称失去持续归属保证，因此不能作为临时发布隔离手段。
...
进入发布隔离时固定执行三项互相独立的配置：停用 Main Mini App、把默认菜单按钮恢复为默认行为、保留 `evomypet` 并把它的 Web App URL 改为当前环境 `/maintenance.html`。禁止使用 `Delete Web App`、`/deleteapp`、临时 short name、第三方占位域名或 Vercel Project 暂停代替。
...
入口恢复必须等待应用、数据库、OpenAPI、Catalog manifest、Battle checksum 与 migration 全部来自同一已验收提交。恢复顺序固定为：先把 `evomypet` URL 改回环境根 URL并用真实 Telegram 直链复核，再恢复 Main Mini App，最后恢复默认菜单按钮；任一步失败都保持其余入口关闭。

## 不变量

- `evomypet` short name、公开直链格式、邀请参数和 Battle `startapp` 参数保持不变；隔离只改变 BotFather 保存的 Web App URL。
...
真实 iPhone Telegram 必须从 `https://t.me/EvoMyPet_bot/evomypet` 打开，完整显示英语与简体中文维护信息，页面不进入游戏、不出现登录或资产请求。BotFather 必须继续列出 `evomypet`，其 Web App URL 精确等于维护页地址；Main Mini App 保持停用，默认菜单按钮保持默认行为。静态文件存在、浏览器直接打开或 deployment `READY` 不能替代该真实直链验收。
