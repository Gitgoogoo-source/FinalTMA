...
产品第 21 章要求在现有游戏页新增 Monster Tamer，同时保持 Expedition 与 Wheel 不变。Monster Tamer 以固定上游提交为基础保留自由探索、NPC、捕捉、战斗、队伍、背包和存档玩法，删除两段强制教学剧情与教授相关对白，并且不属于 FinalTMA 的账号、资产、藏品、奖励或交易体系。
...
12. `New Game` 在 `main_1` 出生点静默把等级 5 的 Iguanignite 放入队伍并立即允许自由探索；`main_1` 不包含 `Events` 触发层，事件数据不包含剧情条目，教授相关 NPC 对白被删除，告示牌、普通 NPC、治疗 NPC 和训练师保持可交互。
...
静态游戏可以独立加载、保存和回滚，FinalTMA 业务安全不依赖其客户端正确性。发布门禁必须验证静态目录完整、launcher 纯链接、路由优先级、业务引用为零、许可证存在和游戏页顺序；功能验收另覆盖自由探索玩法、静默初始怪物、无强制教学剧情、全稳定视口、战斗上下构图、倒下回城镜头一致性、点击目标移动、碰撞停止、键盘与紧凑触控浮层、大小写敏感资源路径和本地存档。
