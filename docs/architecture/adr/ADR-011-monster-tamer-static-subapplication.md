...
静态游戏可以独立加载、保存和回滚，FinalTMA 业务安全不依赖其客户端正确性。发布门禁必须验证静态目录完整、launcher 纯链接、路由优先级、业务引用为零、许可证存在和游戏页顺序；功能验收另覆盖完整上游玩法、全稳定视口、点击目标移动、碰撞停止、键盘与紧凑触控浮层、大小写敏感资源路径和本地存档。
