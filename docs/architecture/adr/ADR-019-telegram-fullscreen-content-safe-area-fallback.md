...
[Telegram Mini Apps 官方协议](https://core.telegram.org/bots/webapps)定义 `safeAreaInset` 表示设备系统安全区，`contentSafeAreaInset` 表示不会被 Telegram 原生控件覆盖的内容安全区。五个主页的身份字首和资产入口以及任务页的页内返回按钮必须位于这两类区域之后。开盒页不展示页内返回按钮。
...
回退值只在 `platform/telegram` 与全局壳层中维护。交易、游戏、开盒、藏品和任务页不得增加独立偏移；身份字首、资产栏、页面内容起点和任务页的页内返回按钮共用同一计算结果。
