...
`tasks.get` 的 19 个 `code`、9 个 `category`、4 个 `status` 和 16 个 `completion_action` 全部使用固定枚举。任务同时返回固定标题、真实条件描述、当前进度、目标和 Gems 奖励。Web 只按枚举映射简体中文，不向用户直接展示任何内部枚举；`completion_action` 只允许切页、切换页签、滚动和聚焦，不调用写接口。
