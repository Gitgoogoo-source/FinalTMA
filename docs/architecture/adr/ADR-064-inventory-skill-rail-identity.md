...
当前选中技能的可见摘要固定由技能名、分隔线和完整伤害整数组成；伤害整数前不显示“Damage”“攻击力”“伤害”或其他属性标签。技能条按钮的无障碍名称继续包含技能名与伤害数值，不改变点选交互或技能数据。

## 不变量

- 同一当前藏品的 `.inventory-hero-art` 恰好只有 1 个 `.inventory-skill-rail`。
- 彩色条数量等于当前模板实际渲染技能数，且始终为 1—4；当前正式模板固定为 2、3 或 4。
- 技能条点选只改变当前技能名、仅含整数的伤害数值和本地选中长度，不读取 API、不写数据库、不提交战斗动作。
