...
操作 UUID 同时是 `Idempotency-Key` 与 `operation_id`。数据库对规范化请求计算 SHA-256；同键同请求回放持久结果，同键不同请求返回 `IDEMPOTENCY_KEY_REUSED`。开盒结果确认时间与原操作同表保存，确认 RPC 锁定当前用户的原开盒终态并使用首次确认时间，重复与并发确认不改变结果。市场购买响应不包含卖家身份；库存满足 `total = available + listed + trading + minting + expedition`。
