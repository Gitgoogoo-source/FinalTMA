...
Battle 分享卡、接受页、lobby、己方、对手和 resolution event 使用六个独立 Schema。lobby DTO 只公开固定 creator/opponent presence，不返回真实头像；固定红蓝 WebP 由 Web 静态资源提供。数据库 RPC 直接生成 viewer-specific JSON，不允许 Functions 取得全量双边状态后再过滤。`battle` 不加入 Exposed schemas；永久私有 seed、roll、审计与 ledger 关联不进入玩家响应。

Battle 创建、取消、接受、正常动作和强制换宠必须携带 UUID `Idempotency-Key`，数据库 request hash 阻止同键篡改与重放产生第二次业务结果。heartbeat、offline 和 acknowledge 禁止携带 `Idempotency-Key` 且不创建 operation；数据库分别以 participant 服务端时间单调更新、受 room 锁保护的幂等 presence 转换和仅空值可写的首次确认时间吸收重复、并发与请求重放。邀请 waiting 的创建者在线只作展示，不参与接受授权；lobby 在线、倒计时、取消和开战只由数据库 participant presence 裁决。
