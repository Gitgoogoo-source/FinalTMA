...
pnpm db:diff:check
pnpm chain:build
python3 tools/catalog/build.py --check-assets
```
...
python3 tools/web/build_manifest.py \
  ...
  --icon-url https://APP_HOST/assets/share/preview.webp \
  --terms-url https://APP_HOST/terms \
  --privacy-url https://APP_HOST/privacy
```

正式素材首次上传后运行 `pnpm catalog:pin-assets`，复核变更并提交 checksum；此后所有环境只运行 `pnpm catalog:check-assets`，不得重新 pin 不一致的文件。

## 3. 测试环境

1. 将三条 migration 按文件名顺序应用到空测试库。
2. 部署同一 Git commit 到测试 Vercel 项目并配置测试 secrets。
3. 发布 testnet collection，记录地址、交易 hash、owner、permit 公钥和 1% 版税验证结果。
4. 配置测试 Bot webhook/Mini App URL。
5. 按 `ops/evidence/acceptance-template.md` 完成 Telegram 真机、支付、并发与 Mint 验收。
6. 执行四个 job；`monitor-invariants` 必须返回 0 个新增 violation。

## 4. 生产切换

顺序不可调整：

1. `python3 tools/catalog/build.py --check-assets` 成功。
2. 再次证明生产 migration history 为空且无须迁移数据。
3. 应用 `20260718000100_baseline.sql`、`20260718000200_catalog_v1.sql`、`20260718000300_api_grants.sql`。
