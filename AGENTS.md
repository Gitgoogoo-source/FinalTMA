...
本文件是编码总规则。执行 vibe coding 时，先读本索引，再按当前任务读取相关小节；不要因为存在长文档就一次性全文读取。

### 0.1 默认读取顺序

1. 先读取本文件的“0. AI 读取索引优先规则”。
2. 再根据任务关键词，搜索并读取本文件相关小节。
3. 需要项目结构时，先读 `项目完整结构树.md` 的 AI 读取索引，再搜索具体目录或文件名。
4. 需要结构责任说明时，先读 `项目结构树说明.md` 的 AI 读取索引，再搜索相关模块。
5. 需要界面和功能说明时，先读 `项目功能与界面说明.md` 的 AI 读取索引，再搜索相关界面。
6. 需要第一阶段实现步骤时，先读 `第一阶段教学指南.md` 的 AI 读取索引，再搜索具体步骤。

### 0.2 禁止全文读取规则

- 禁止在没有明确任务目标时全文读取 `项目完整结构树.md`、`项目结构树说明.md`、`项目功能与界面说明.md`、`第一阶段教学指南.md`。
- 如果某个文档超过 300 行，必须先搜索关键词或标题，只读取命中的小节和相邻上下文。
- 只有在用户明确要求“全文审计、全文整理、完整迁移检查、完整结构核对”时，才允许全文读取长文档。
- 如果搜索结果指向多个合理模块，必须先停下来让用户选择，不要猜测。

### 0.3 任务关键词到文档映射

| 任务关键词 | 优先读取 |
|---|---|
| 目录、文件、结构、路径、落点 | `项目完整结构树.md`、`项目结构树说明.md` |
| 技术栈、架构、模块责任、数据库 schema、RPC、API 职责 | `项目结构树说明.md` |
| 资产栏、开盒、交易、藏品、图鉴、任务、游戏、弹窗 | `项目功能与界面说明.md` |
| 第一阶段、基础闭环、开发步骤、验收、测试、部署 | `第一阶段教学指南.md` |
| 编码规则、安全、隐私、环境变量、前后端边界 | `AGENTS.md` |

### 0.4 隐私和环境变量提醒

前端代码不可信。真实地址、TON 钱包、Bot Token、Supabase service role key、TON 私钥、管理员密钥和其他密钥，必须放在 Vercel 服务端环境变量中，不能放进前端代码或前端可读取配置。
