...
当前还没有 `/album` 路由。开发图鉴时需要新增 `/album`，同时补齐 routes 常量和底部导航入口。
...
- `inventory.item_instances`：已有用户藏品实例
- `inventory.item_instance_events`：已有库存事件
- `inventory.inventory_locks`：已有锁记录
- `album.user_discoveries`：已有永久图鉴发现记录
...
开发前需要先检查数据库中 RPC 是否仍存在。已知项目中存在或规划过以下 RPC：
...
- `api.album_get_items`，可合并进 progress
...
1. 检查目标文件是否已存在。
2. 检查同类模块现有写法。
3. 检查 validation schema 是否已定义。
4. 检查 API_ENDPOINTS 是否已有路径。
5. 检查 queryKeys 是否已有缓存 key。
6. 检查 RPC 签名是否和 API 调用一致。
7. 检查数据库表是否已有规则数据。
8. 检查操作是否会影响交易锁。
9. 检查成功后需要 invalidate 哪些 query。
10. 检查失败时是否会产生脏数据。
...
api/album/items.ts            # 图鉴物品列表；可并入 progress
...
EvolutionPanel.tsx            # 3 材料选择、成功率、目标形态
