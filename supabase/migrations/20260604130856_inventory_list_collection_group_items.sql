...
create or replace function api.inventory_list_collection_group_items(
  p_user_id uuid,
  p_template_id uuid,
...
begin
  ...
  with filtered as (
    ...
    where ii.owner_user_id = p_user_id
      and ii.template_id = p_template_id
      and (
        (p_form_id is null and ii.form_id is null)
        or ii.form_id = p_form_id
      )
  ...
  ) order by level desc, power desc, acquired_at desc, serial_no desc, id desc), '[]'::jsonb)
  into v_items
  from filtered;
...
revoke execute on function api.inventory_list_collection_group_items(uuid, uuid, uuid, text[], integer, integer) from public, anon, authenticated;
grant execute on function api.inventory_list_collection_group_items(uuid, uuid, uuid, text[], integer, integer) to service_role;
