...
begin
  ...
  numbered_rows as (
    select
      *,
      row_number() over (order by created_at desc, activity_id desc) as row_no
    from filtered_rows
  )
  select
    coalesce(
      jsonb_agg(
        ...
        order by created_at desc, activity_id desc
      ) filter (where row_no <= v_limit),
      '[]'::jsonb
    ),
    count(*) > v_limit,
    max(created_at::text || '|' || activity_id::text) filter (where row_no = v_limit)
