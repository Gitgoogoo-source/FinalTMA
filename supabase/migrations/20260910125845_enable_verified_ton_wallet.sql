...
begin
  ...
  begin
    perform pg_advisory_xact_lock(hashtextextended('wallet.user:' || v_user_id::text, 0));
    perform pg_advisory_xact_lock(hashtextextended('wallet.address:' || p_address, 0));
    ...
    if exists (select 1 from onchain.wallets where address = p_address and user_id <> v_user_id) then perform api.raise_business_error('WALLET_ADDRESS_IN_USE', '该地址已绑定其他账号'); end if;
    ...
    where onchain.wallets.user_id = v_user_id
    returning * into v_wallet;
    if v_wallet.id is null then perform api.raise_business_error('WALLET_ADDRESS_IN_USE', '该地址已绑定其他账号'); end if;
