-- Step 6 RPC gap fixes.
-- Adds the first-stage DEV paid gacha RPC and the inventory listing RPC required
-- by 第一阶段教学指南.md. Existing formal Stars processing stays in
-- api.gacha_process_paid_order.
...
revoke execute on function api.gacha_process_dev_paid_order(uuid, uuid) from public, anon, authenticated;
revoke execute on function api.inventory_list_user_items(uuid, text[], integer, integer) from public, anon, authenticated;
grant execute on function api.gacha_process_dev_paid_order(uuid, uuid) to service_role;
