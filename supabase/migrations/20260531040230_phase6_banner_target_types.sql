-- Phase 6: normalize activity banner targets to the guide contract.
-- Keep target_ref for existing frontend/API compatibility, and add a
-- structured target_payload column for validated target metadata.

alter table catalog.banner_campaigns
  add column if not exists target_payload jsonb not null default '{}'::jsonb;

alter table catalog.banner_campaigns
  drop constraint if exists banner_campaigns_target_type_check;

update catalog.banner_campaigns
set target_type = case target_type
    when 'market_listing' then 'listing'
    when 'external_url' then 'external'
    when 'shop_product' then 'payment'
    else target_type
  end,
  target_payload = case target_type
    when 'box' then jsonb_build_object('box_id', target_ref)
    when 'market_listing' then jsonb_build_object('listing_id', target_ref)
    when 'listing' then jsonb_build_object('listing_id', target_ref)
    when 'task' then jsonb_build_object('task_ref', target_ref)
    when 'shop_product' then jsonb_build_object('star_order_id', target_ref)
    when 'payment' then jsonb_build_object('star_order_id', target_ref)
    when 'external_url' then jsonb_build_object('url', target_ref)
    when 'external' then jsonb_build_object('url', target_ref)
    else '{}'::jsonb
  end,
  updated_at = now()
...
alter table catalog.banner_campaigns
  add constraint banner_campaigns_target_type_check
  check (target_type in ('box', 'listing', 'task', 'payment', 'external', 'none'));

alter table catalog.banner_campaigns
  drop constraint if exists banner_campaigns_target_payload_object_check;

alter table catalog.banner_campaigns
  add constraint banner_campaigns_target_payload_object_check
  check (jsonb_typeof(target_payload) = 'object');

drop function if exists api.admin_upsert_banner_campaign(
...
declare
  ...
  v_target_payload jsonb := coalesce(p_target_payload, '{}'::jsonb);
...
begin
  ...
  if v_target_type not in ('box', 'listing', 'task', 'payment', 'external', 'none') then
  ...
  if jsonb_typeof(v_target_payload) <> 'object' then
    raise exception 'ADMIN_BANNER_TARGET_PAYLOAD_INVALID' using errcode = 'P0001';
  end if;
  ...
  if v_target_type = 'none' then
    if v_target_ref is not null or v_target_payload <> '{}'::jsonb then
    ...
    v_target_ref := coalesce(
      v_target_ref,
      nullif(trim(coalesce(v_target_payload ->> 'box_id', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'id', '')), '')
    );

    if v_target_ref is null or v_target_ref !~* v_uuid_pattern then
      raise exception 'ADMIN_BANNER_TARGET_REF_INVALID' using errcode = 'P0001';
    end if;

    if not exists (
      select 1
      from gacha.blind_boxes b
      where b.id = v_target_ref::uuid
        and b.status in ('not_started', 'active', 'paused', 'sold_out')
    ) then
      raise exception 'ADMIN_BANNER_TARGET_NOT_FOUND' using errcode = 'P0001';
    end if;

    v_target_payload := jsonb_build_object('box_id', v_target_ref);
  elsif v_target_type = 'listing' then
    v_target_ref := coalesce(
      v_target_ref,
      nullif(trim(coalesce(v_target_payload ->> 'listing_id', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'id', '')), '')
    );
    ...
    v_target_payload := jsonb_build_object('listing_id', v_target_ref);
  elsif v_target_type = 'task' then
    v_target_ref := coalesce(
      v_target_ref,
      nullif(trim(coalesce(v_target_payload ->> 'task_id', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'task_code', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'task_ref', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'id', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'code', '')), '')
    );

    if v_target_ref is null then
    ...
    v_target_payload := jsonb_build_object('task_ref', v_target_ref);
  elsif v_target_type = 'payment' then
    if v_target_payload ?| array['status', 'payment_status', 'paid', 'success', 'fulfilled'] then
      raise exception 'ADMIN_BANNER_PAYMENT_TARGET_PAYLOAD_INVALID' using errcode = 'P0001';
    end if;

    v_target_ref := coalesce(
      v_target_ref,
      nullif(trim(coalesce(v_target_payload ->> 'star_order_id', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'payment_order_id', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'id', '')), '')
    );
    ...
    if not exists (
      ...
      where so.id = v_target_ref::uuid
        and so.status in ('created', 'invoice_created', 'precheckout_ok', 'precheckout_checked')
        and (so.expires_at is null or so.expires_at > v_now)
    ) then
      raise exception 'ADMIN_BANNER_TARGET_NOT_FOUND' using errcode = 'P0001';
    end if;

    v_target_payload := jsonb_build_object('star_order_id', v_target_ref);
  elsif v_target_type = 'external' then
    v_target_ref := coalesce(
      v_target_ref,
      nullif(trim(coalesce(v_target_payload ->> 'url', '')), ''),
      nullif(trim(coalesce(v_target_payload ->> 'href', '')), '')
    );
    ...
    v_target_payload := jsonb_build_object('url', v_target_ref);
  end if;
  ...
  v_request_hash := jsonb_build_object(
    ...
    'target_payload', v_target_payload,
    'status', v_status,
  ...
  v_response := jsonb_build_object(
    ...
    'target_payload', v_campaign.target_payload,
...
revoke all on function api.admin_upsert_banner_campaign(
  uuid, text, text, text, text, text, text, text, text, jsonb, uuid, text, text, jsonb, timestamptz, timestamptz, integer, jsonb
...
comment on function api.admin_upsert_banner_campaign(
  uuid, text, text, text, text, text, text, text, text, jsonb, uuid, text, text, jsonb, timestamptz, timestamptz, integer, jsonb
) is 'Create or update a banner campaign with phase 6 target validation, canonical target payload, idempotency and audit logging.';
