-- Phase 4 3.4A: make daily check-in drive task progress through the shared
-- task progress RPC selected for this implementation path.
...
begin
  if to_regprocedure('api.task_record_progress(uuid,text,integer,uuid,text)') is null then
    raise exception 'api.task_record_progress(uuid,text,integer,uuid,text) is required before applying phase4_signin_records_task_progress';
  end if;
end $$;
