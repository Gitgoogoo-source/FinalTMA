-- Phase 4 / 11.5 task reward ledger reconciliation acceptance checks.
...
with campaign as (
  ...
  values (
    'SIGNIN_LEDGER_11_5_TEST',
    '11.5 Ledger Sign-in Test',
    'pgTAP sign-in ledger fixture',
    7,
    true,
    now() - interval '1 day',
    now() + interval '7 days',
    '{"test":"phase4_11_5_ledger"}'::jsonb
  )
...
with task_row as (
  ...
  values (
    'TASKS_LEDGER_11_5_CLAIM_TEST',
    'daily',
    '11.5 Ledger Claim Test',
...
insert into _ids (key, id) values
  ('task_user', testutil.make_user(11110500001, 'tasks_ledger_11_5_task_user', null)),
  ('signin_user', testutil.make_user(11110500002, 'tasks_ledger_11_5_signin_user', null)),
  ('inviter', testutil.make_user(11110500003, 'tasks_ledger_11_5_inviter', null)),
  ('invitee', testutil.make_user(11110500004, 'tasks_ledger_11_5_invitee', null));
...
select 'commission_claim', api.referral_claim_commission(
  ...
  'tasks-ledger-11-5-commission-claim'
);
  ...
  (
    with reward_totals as (
      select
        ...
        claim.user_id,
        upper(coalesce(item.value ->> 'currency', item.value ->> 'currency_code')) as currency_code,
        sum(nullif(item.value ->> 'amount', '')::numeric) as reward_amount
      from tasks.task_claims claim
      cross join lateral jsonb_array_elements(coalesce(claim.reward, '[]'::jsonb)) as item(value)
      where coalesce(item.value ->> 'currency', item.value ->> 'currency_code') is not null
        and coalesce(nullif(item.value ->> 'amount', '')::numeric, 0) > 0
      group by claim.id, claim.user_id, upper(coalesce(item.value ->> 'currency', item.value ->> 'currency_code'))
    ),
    ledger_totals as (
      select
        ledger.source_id,
        ...
        sum(ledger.amount) as ledger_amount
      from economy.currency_ledger ledger
      where ledger.source_type = 'task_claim'
        and ledger.entry_type = 'credit'
      group by ledger.source_id, ledger.user_id, ledger.currency_code
    )
    select count(*)::int
    from reward_totals reward
    left join ledger_totals ledger
      on ledger.source_id = reward.source_id
     and ledger.user_id = reward.user_id
     and ledger.currency_code = reward.currency_code
    where ledger.ledger_amount is distinct from reward.reward_amount
  ),
  0,
  'all task_claims currency rewards reconcile to task_claim ledger totals'
);
...
select is(
  (
    with reward_totals as (
      select
        signin.id as source_id,
        signin.user_id,
        upper(coalesce(item.value ->> 'currency', item.value ->> 'currency_code')) as currency_code,
        sum(nullif(item.value ->> 'amount', '')::numeric) as reward_amount
      from tasks.user_signins signin
      cross join lateral jsonb_array_elements(coalesce(signin.reward, '[]'::jsonb)) as item(value)
      where coalesce(item.value ->> 'currency', item.value ->> 'currency_code') is not null
        and coalesce(nullif(item.value ->> 'amount', '')::numeric, 0) > 0
      group by signin.id, signin.user_id, upper(coalesce(item.value ->> 'currency', item.value ->> 'currency_code'))
    ),
    ledger_totals as (
      select
        ledger.source_id,
        ...
        sum(ledger.amount) as ledger_amount
      from economy.currency_ledger ledger
      where ledger.source_type = 'daily_check_in'
        and ledger.entry_type = 'credit'
      group by ledger.source_id, ledger.user_id, ledger.currency_code
    )
    select count(*)::int
  ...
  0,
  'all user_signins currency rewards reconcile to daily_check_in ledger totals'
);
...
select is(
  (
    select count(*)::int
    from tasks.referral_rewards reward
    left join economy.currency_ledger ledger on ledger.id = reward.ledger_id
    where reward.status = 'granted'
      and (
        reward.ledger_id is null
        or ledger.id is null
        or ledger.user_id is distinct from reward.user_id
        or ledger.currency_code is distinct from reward.currency_code
        or ledger.amount is distinct from reward.amount
        or ledger.entry_type is distinct from 'credit'
        or ledger.source_type is distinct from 'referral_first_open'
      )
  ),
  0,
  'all granted referral rewards point to matching first-open ledger rows'
);

select is(
  (
    select count(*)::int
    from tasks.referral_commissions commission
    left join economy.currency_ledger ledger on ledger.id = commission.ledger_id
    where commission.status = 'granted'
      and (
        commission.ledger_id is null
        or ledger.id is null
        or ledger.user_id is distinct from commission.inviter_user_id
        or ledger.currency_code is distinct from 'KCOIN'
        or ledger.amount is distinct from commission.commission_amount_kcoin
        or ledger.entry_type is distinct from 'credit'
        or ledger.source_type is distinct from 'referral_commission_claim'
      )
  ),
  0,
  'all granted referral commissions point to matching claim ledger rows'
);
...
select is(
  (
    select count(*)::int
    from economy.user_balances balance
    left join lateral (
      select coalesce(sum(
        case
          when ledger.entry_type in ('credit', 'refund', 'unlock', 'adjustment') then ledger.amount
          when ledger.entry_type in ('debit', 'fee', 'lock') then -ledger.amount
          when ledger.entry_type = 'reversal' then 0
          else 0
        end
      ), 0)::numeric as signed_available
      from economy.currency_ledger ledger
      where ledger.user_id = balance.user_id
        and ledger.currency_code = balance.currency_code
    ) ledger on true
    where balance.available_amount is distinct from ledger.signed_available
  ),
  0,
  'all user_balances available snapshots equal signed currency_ledger totals'
);
