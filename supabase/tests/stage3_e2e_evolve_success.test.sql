...
values (
  'target_catalog',
  testutil.stage3_e2e_evolve_create_catalog_fixture('stage3-e2e-evolve-target')
);
...
select ok(exists (
  ...
  where id = (select id from _ids where key = 'result_item')
    ...
    and template_id = ((select payload from _ids where key = 'target_catalog') ->> 'template_id')::uuid
    and form_id = ((select payload from _ids where key = 'target_catalog') ->> 'base_form_id')::uuid
    and status = 'available'
    and source_type = 'evolution'
), 'database creates a new available target item');
