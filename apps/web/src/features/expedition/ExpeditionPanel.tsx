...
export function ExpeditionPanel(): ReactNode {
  ...
  const usedToday = query.data?.used_today ?? { normal: 0, intermediate: 0, advanced: 0 };
  ...
  const expectedReward = items.reduce(
    (sum, item) =>
      sum +
      item.unit_reward_fgems * (selection[item.template_id] ?? 0),
    0,
  );
  useEffect(() => {
          ...
          {rules.map((rule) => {
            ...
            return (
              <Card key={tier} className="expedition-card">
                ...
                <small>
                  {rule.allowed_rarities.map((rarity) => rarityNames[rarity] ?? rarity).join(" · ")}
                </small>
                <span>
                  {rule.duration_minutes} 分钟 · 今日 {usedToday[tier]}/{rule.daily_limit}
                </span>
                {running ? (
                  ...
                  return (
                    <Card key={id} className={count ? "selected" : ""}>
                      ...
                      <div>
                        ...
                        <small>
                          {item.rarity} · 第 {item.stage} 阶 · {item.unit_reward_fgems} Fgems
                        </small>
                      </div>
                      <Button
                        disabled={count === 0}
                      ...
                      <Button
                        disabled={
                          selectedCount >= 3 || count >= item.available
                        }
                        onClick={() =>
                          setSelection((value) => ({
                            ...value,
