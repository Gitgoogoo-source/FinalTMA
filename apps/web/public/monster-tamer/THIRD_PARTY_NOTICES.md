...
Every file under `assets/images/monster-tamer/**` and `favicon.ico` is an
original replacement created for this integration. The monster portraits,
title landscape, battle landscape, and two trainer illustrations were created
from text-only prompts without an upstream image reference. The maps, UI,
capture items, and favicon were drawn deterministically by
`tools/monster-tamer/generate-original-assets.mjs`.

The generated maps use only the fixed gameplay JSON's collision, encounter,
transition, item, sign, and foreground coordinates so gameplay geometry remains
intact. No uncleared upstream Monster Tamer raster remains in the published
tree. See `ORIGINAL_ASSET_PROVENANCE.md` for the prompt and output record.
