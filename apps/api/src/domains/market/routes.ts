...
export const marketHandlers = {
  "market.bootstrap": async (context) => ({
    data: await rpc("market_bootstrap", {
      p_session_id: requireSession(context).session_id,
    }),
  }),
  "market.template": async (context) => ({
    data: await rpc("market_template", {
      p_session_id: requireSession(context).session_id,
  ...
  "market.create_listing": async (context) =>
    operationResult(
      await rpc<OperationEnvelope>("market_create_listing", {
        p_session_id: requireSession(context).session_id,
  ...
  "market.purchase": async (context) =>
    operationResult(
      await rpc<OperationEnvelope>("market_purchase", {
        p_session_id: requireSession(context).session_id,
        p_operation_id: requireOperationId(context),
        p_template_id: context.input.template_id,
        p_quantity: context.input.quantity,
