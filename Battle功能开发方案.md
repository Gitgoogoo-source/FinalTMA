...
11. 页面隐藏、Telegram deactivated、`pagehide` 或离开 `/game` 立即结束当前 lease、中止在途 heartbeat 并尽力 offline；恢复必须先读取权威快照，已结束 lifecycle 必须取得数据库认可的新 lease，仍 active 的 lifecycle 继续单调序号。当前 `waiting` 创建者实际打开原生分享面板后，属于同一 session generation 与 room 的 `shareMessage` callback、`shareMessageSent` 或 `shareMessageFailed` 固定作为面板完成信号；若 `/game` 已可见则立即恢复，若信号先到则保留到紧随其后的可见、页面显示、聚焦或激活事件再恢复。即使 Telegram 没有补发 `activated` 且 `isActive` 仍为 `false`，Web 也以该信号触发同一权威恢复路径。新 lease 接管后旧 heartbeat/offline 永久无副作用，安全不依赖 abort、offline 必达或平台补发激活事件。
...
- 五档稀有度强度系数。
- 14 个基础角色档案。
- 10 个技能数值槽位及三类进化链的技能位置有效威力下限。
- 14 组四技能候选池及其基础 power 成长顺序。
- 70 条进化链的属性和角色档案映射。
...
同稀有度的所有档案预算完全相同。高稀有度拥有更高预算，属于明确的真实战斗优势。`combat_power` 和阶段编号不作为隐藏倍率参与 Battle 计算；链类型只通过 5.7 节公开、构建期固化的技能位置有效威力下限产生作用，不存在运行时隐藏倍率。进化带来的实际强化由新模板对应的更高稀有度四维和按阶段固定增加的技能共同体现。
...
全部技能都是同级主动直接伤害技能，只由模板有效威力与命中率形成数值差异；技能不改变行动顺序，也没有基础/战术/强力分类、被动、冷却、次数、能量、异常状态或能力变化。下表威力是候选池排序使用的基础威力。
...
模板实际显示和结算使用的有效威力为 `max(基础威力, 对应链类型与技能位置的下限)`：

| 进化链类型 | 对应档案 | 位置 1 | 位置 2 | 位置 3 | 位置 4 |
| ---------- | -------- | -----: | -----: | -----: | -----: |
| 普通链     | P01—P08  |      0 |      0 |      0 |      0 |
| 高级链     | P09—P12  |     71 |     81 |     96 |    126 |
| 顶级链     | P13—P14  |     72 |     82 |    106 |    127 |

这些下限是只上调、不削弱现有技能且满足同阶段同位置跨稀有度严格递增的最小整数。模板的技能 ID 与有效威力一同进入版本化配置和房间快照；前端不得计算或补全。
...
生成器按 `(数值槽位基础 power 升序, 候选槽位原始位置升序)` 把四个候选技能整理为唯一成长顺序，再按 5.7 节的链类型位置下限固化模板有效威力。模板实际技能数固定为 `stage + 1`：1 阶取前 2 个、2 阶取前 3 个、3 阶取全部 4 个；写入模板、房间快照和 API 时位置连续为 `1..N`。同链阶段技能 ID 与有效威力都必须保持前缀继承；同阶段、同技能位置的有效威力必须随稀有度严格升高。

玩家不能学习、替换、手动解锁或重排技能。未拥有技能不进入返回数组、DOM 或锁定占位。模板不保证拥有 100% 命中技能，生成器不得为命中率改变 power 顺序；L06、L13 的 1 阶模板允许只有 90%/95% 命中技能。

### 5.10 确定伤害公式

命中后的计算只读取攻击者房间快照中的实际攻击 `A`、防守者实际防御 `D`、本次技能位置的模板有效威力 `P` 和属性倍率 `T_bps`：
...
全部计算使用 PostgreSQL `bigint` 的固定运算顺序。单次命中伤害不超过防守宠物最大生命的 80%，因此满生命宠物不会被一招击倒；当前生命已经低于伤害时仍正常被击倒。公式没有等级、`combat_power`、运行时阶段加成、同属性加成、暴击、随机区间、状态或客户端参数。攻击、防御、属性克制和单次伤害上限仍可改变最终扣血；跨稀有度严格递增只约束同阶段、同技能位置的模板有效威力，不承诺任意对局的最终扣血恒定排序。
...
原生分享弹窗被用户关闭或发送失败不会自动取消已经激活的等待房；创建者继续重试分享或主动取消房间。callback 或官方 sent/failed 事件只确认本次原生面板流程已经结束：Web 在当前页面可见时先回正 room，已经结束的 lifecycle 恢复新 lease，仍 active 的 lease 继续单调序号；分享成功或失败文案仍只属于本地反馈，不写入房间、资产或业务状态。
...
“游戏”页只渲染八种权威页面状态：Battle 首页、三槽队伍选择、挑战卡准备、等待、接受、双人 lobby、战斗、当场结果。Battle 首页按主功能文档第 21.3 节的唯一映射展示“选择战场”及三个横向档位卡片，不展示战场等级。每张卡片左侧正式 Battle 房间美术、三分之二宽度和中心裁切保持不变；右侧三分之一只显示档位稀有度、真实数量的 1 / 2 / 3 枚宝石、双方奖池及箭头或勾选，不显示战场名称或每人门票。稀有度文字、宝石、奖池标签和奖池数值统一使用当前档位的绿 / 橙 / 紫强调色；宝石不使用灰色补足三枚，当前选择仍使用项目原橘色边框和橘色勾选。这些展示元数据不进入 API 命令、匹配隔离、战斗配置或经济结算。三槽队伍选择固定提供“邀请好友”和“随机匹配”；`battle.team_options` 首次读取完成前固定显示“正在读取本人可用藏品”并禁用两个确定动作，只有读取完成且当前筛选没有可见项时才显示“没有符合条件的可用藏品”。同一 `waiting` 页面按 `room_mode` 分别渲染 30 分钟好友分享等待或 120 秒同档公开匹配等待。战斗页同时承载普通行动和倒下后的换宠反击，不存在独立换宠页面。
...
- 每模板技能位置连续、有效威力非递减、元素匹配且无重复；有效威力不得低于对应数值槽位基础威力；不校验每模板拥有 100% 命中技能。
- 同链三个阶段的已有技能 ID 与有效威力严格保持前缀；同阶段每个共有技能位置的稀有度相邻档最小值都严格大于低一档最大值。
- 所有同稀有度模板预算相同。
- 每个档案的五档四维逐档严格上升。
