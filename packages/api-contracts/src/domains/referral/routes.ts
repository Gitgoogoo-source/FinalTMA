...
const referralOutput = z
  .object({
    ...
    link: z.string().url(),
    share_text: z.string(),
    bound_friends: z.number().int().min(0),
    valid_recharge_friends: z.number().int().min(0),
    reward_fgems_total: z.number().int().min(0),
    rewarded_today: z.number().int().min(0).max(20),
    rewarded_lifetime: z.number().int().min(0).max(300),
    milestone_5_status: z.enum(["pending", "granted"]),
