...
const tonProofSchema = z
  .object({
    ...
    domain: z
      .object({
        length_bytes: z.number().int().positive(),
...
export const walletRoutes = [
  defineRoute({
    ...
    output: walletStatusSchema,
    errors: ["ACCOUNT_RESTRICTED", "INTERNAL_ERROR"],
  }),
  defineRoute({
    id: "wallet.challenge",
    method: "POST",
    path: "/api/wallet/challenges",
    gateway: "app",
    auth: true,
    idempotent: false,
    input: emptyObjectSchema,
    output: z
      .object({ payload: z.string(), expires_at: timestampSchema })
      .strict(),
    errors: ["ACCOUNT_RESTRICTED", "INTERNAL_ERROR"],
  }),
  defineRoute({
    ...
    input: z
      .object({
        ...
        wallet_app_name: z.string().max(128).nullable(),
      })
      .strict(),
    output: walletStatusSchema
      .extend({
