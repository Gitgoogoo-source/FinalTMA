...
export const expeditionInputItemSchema = z
  .object({ template_id: z.string().min(1), quantity: z.number().int().positive() })
  .strict();
...
export const paymentSchema = z
  .object({
    ...
    status: z.enum(["pending", "paid", "delivered", "expired", "refunded", "rejected"]),
...
export const mintSchema = z
  .object({
    id: uuidSchema,
    template_id: z.string(),
    status: z.enum(["reserved", "submitted", "unknown", "succeeded", "failed", "cancelled"]),
