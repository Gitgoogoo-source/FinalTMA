...
export const gachaHandlers = {
  "gacha.bootstrap": async (context) => ({ data: await rpc("gacha_bootstrap", { p_session_id: requireSession(context).session_id }) }),
  "gacha.open": async (context) => operationResult(await rpc<OperationEnvelope>("gacha_open", {
