...
适用项目：`tmaGame` Telegram Mini App 开盲盒抽卡小游戏  
建议文件名：`AGENTS.md`、`AI_CODING_RULES.md`、`.cursorrules` 或 `CLAUDE.md`  
目标：让 AI 编码助手在编写、修改、重构项目代码时，减少幻觉，严格遵守项目架构、业务边界、安全规则和数据库事务原则。
...
- 用户资产展示：头像、K-coin、Fgems、Stars、TON 钱包状态。
