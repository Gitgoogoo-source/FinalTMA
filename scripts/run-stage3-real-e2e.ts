...
if (statusResult.status !== 0) {
  ...
  process.stderr.write(
    "\nUnable to read local Supabase status. Start Supabase locally with `pnpm db:start` before running Stage 3 real E2E tests.\n",
  );
...
const testResult = spawnSync(
  ...
  [
    ...
    "test",
    "--config",
    "playwright.stage3-real.config.ts",
  ],
  {
    stdio: "inherit",
    env: {
      ...process.env,
      STAGE3_REAL_E2E: "1",
      STAGE3_REAL_E2E_SUPABASE_URL: supabaseUrl,
      STAGE3_REAL_E2E_SUPABASE_ANON_KEY: anonKey,
      STAGE3_REAL_E2E_SUPABASE_SERVICE_ROLE_KEY: serviceRoleKey,
      STAGE3_REAL_E2E_BOT_TOKEN: botToken,
    },
  },
);
